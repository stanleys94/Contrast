...
namespace CONTRAST_WEB.Controllers
{
    public class InvoiceDownloadController : Controller
    {
        ...
        [Authorize]
        [Authorize(Roles = "contrast.user")]
        public async Task<ActionResult> DownloadAll(DateTime? start, DateTime? end, string search, int outstanding, string all_download = "")
        {
            var identity = (ClaimsIdentity)User.Identity;
            string[] claims = identity.Claims.Where(c => c.Type == ClaimTypes.Role).Select(c => c.Value).ToArray();
            ViewBag.Privillege = claims;
            ...
            int down = 0;
            int looping = 1, start_loop = 0;
            List<int> Counter = new List<int>();

            List<tb_r_travel_request> BTA = new List<tb_r_travel_request>();
            List<InvoiceHelper> newList = new List<InvoiceHelper>();
            tb_m_employee_source_data pos = await GetData.GetDivisionSource(Convert.ToInt32(identity.Name));
            ...
            if (all_download != null)
            {
                ...
                {
                    looping = Filter.Count;
                    start_loop = outstanding;
                }
            }
            for (int i = start_loop; i < looping; i++)
            {
                vw_invoice_actualcost_new newModel = new vw_invoice_actualcost_new();
                bool receipt_issued = false;
                InvoiceHelper temp = new InvoiceHelper();
                down = i;

                if (all_download != null)
                {
                    if (all_download == "issued") receipt_issued = true;
                }

                if (pos.Departemen.Contains("GENERAL") || receipt_issued || down >= outstanding)
                {

                    List<tb_r_invoice_actualcost> placeholder = await GetData.TableInvoiceActualcostSingle(Filter[down].invoice.group_code, Filter[down].invoice.jenis_transaksi);
                    tb_r_invoice_actualcost holder = new tb_r_invoice_actualcost();
                    foreach (var item in placeholder)
                    {
                        if (Filter[down].invoice.vendor_code.Contains(item.vendor_code.ToString())) holder = item;
                        vw_invoice_actualcost_new destination_holder = new vw_invoice_actualcost_new();
                        if (item.destination_1 != null) destination_holder.destination_1 = item.destination_1;
                        if (item.destination_2 != null) destination_holder.destination_2 = item.destination_2;
                        if (item.destination_3 != null) destination_holder.destination_3 = item.destination_3;
                ...
                {
                    newModel = await GetData.InvoiceActualCostNewID(Convert.ToInt32(Filter[down].invoice.id_data));
                }
                BTA = await GetData.TravelRequestGCList(newModel.group_code);
                List<Departure_City> from = new List<Departure_City>();
                Departure_City fromTemp = new Departure_City();
                temp.loged_employee = Filter[down].loged_employee;
                temp.invoice = new vw_invoice_actualcost_new();
                temp.invoice.amount_total = newModel.amount_total;
                temp.invoice.cost_center = newModel.cost_center;
                temp.invoice.create_date = newModel.create_date;
            ...
            for (int loop = 0; loop < newList.Count; loop++)
            {
                tb_r_invoice_actualcost saved = new tb_r_invoice_actualcost();
                tb_r_invoice_actualcost updated = new tb_r_invoice_actualcost();
                List<tb_r_invoice_actualcost> List_Model = new List<tb_r_invoice_actualcost>();

                vw_invoice_actualcost_new newModel1 = new vw_invoice_actualcost_new();

                bool copy = false, copyGA = false, update = false;
                List<vw_invoice_actualcost_new> ListModel = new List<vw_invoice_actualcost_new>();
                if (pos.Departemen.Contains("GENERAL"))
                {
                    List_Model = await GetData.TableInvoiceActualcostSingle(newList[loop].invoice.group_code, newList[loop].invoice.jenis_transaksi);
                    foreach (var item in List_Model)
                    {
                        vw_invoice_actualcost_new insert = new vw_invoice_actualcost_new();
                        insert.amount_1 = item.amount_1;
                ...
                {
                    ListModel = await GetData.InvoiceActualCostNewBTA(newList[loop].invoice.group_code, newList[loop].invoice.jenis_transaksi);
                }
                foreach (var item in ListModel)
                {
                    if (item.vendor_code == newList[loop].invoice.vendor_code) newModel1 = item;
                }
                List<tb_r_invoice_actualcost> list = await GetData.TableInvoiceActualcostSingle(newList[loop].invoice.group_code, newList[loop].invoice.jenis_transaksi);
                ...
                if (list.Count() > 0)
                {
                    ...
                    for (int k = 0; k < list.Count(); k++)
                    {
                        if (list[k].vendor_code == Convert.ToInt32(newList[loop].invoice.vendor_code))
                        {
                            index = k;
                            exist = true;
                        }
                    ...
                    if (exist)
                    {
                        ...
                        if (list[index].amount_total == newList[loop].invoice.amount_total)
                        {
                ...
                if (update || saved.id_data == null)
                {
                    if (newModel1.amount_1 != null) saved.amount_1 = newModel1.amount_1;
                    if (newModel1.amount_2 != null) saved.amount_2 = newModel1.amount_2;
                    if (newModel1.amount_3 != null) saved.amount_3 = newModel1.amount_3;
                    if (newModel1.amount_4 != null) saved.amount_4 = newModel1.amount_4;
                    if (newModel1.amount_5 != null) saved.amount_5 = newModel1.amount_5;
                    if (newModel1.amount_6 != null) saved.amount_6 = newModel1.amount_6;
                    if (newModel1.amount_total != null) saved.amount_total = newModel1.amount_total;
                    if (newModel1.bank_account != null) saved.bank_account = newModel1.bank_account;
                    if (newModel1.cost_center != null) saved.cost_center = newModel1.cost_center;
                    if (newModel1.create_date != null) saved.create_date = newModel1.create_date;
                    if (newModel1.destination_1 != null) saved.destination_1 = newModel1.destination_1;
                    if (newModel1.destination_2 != null) saved.destination_2 = newModel1.destination_2;
                    if (newModel1.destination_3 != null) saved.destination_3 = newModel1.destination_3;
                    if (newModel1.destination_4 != null) saved.destination_4 = newModel1.destination_4;
                    if (newModel1.destination_5 != null) saved.destination_5 = newModel1.destination_5;
                    if (newModel1.destination_6 != null) saved.destination_6 = newModel1.destination_6;
                    if (newModel1.Duration_1 != null) saved.Duration_1 = newModel1.Duration_1;
                    if (newModel1.Duration_2 != null) saved.Duration_2 = newModel1.Duration_2;
                    if (newModel1.Duration_3 != null) saved.Duration_3 = newModel1.Duration_3;
                    if (newModel1.Duration_4 != null) saved.Duration_4 = newModel1.Duration_4;
                    if (newModel1.Duration_5 != null) saved.Duration_5 = newModel1.Duration_5;
                    if (newModel1.Duration_6 != null) saved.Duration_6 = newModel1.Duration_6;
                    if (newModel1.employee_input != null) saved.employee_input = newModel1.employee_input;
                    if (newModel1.endDate_1 != null) saved.endDate_1 = newModel1.endDate_1;
                    if (newModel1.endDate_2 != null) saved.endDate_2 = newModel1.endDate_2;
                    if (newModel1.endDate_3 != null) saved.endDate_3 = newModel1.endDate_3;
                    if (newModel1.endDate_4 != null) saved.endDate_4 = newModel1.endDate_4;
                    if (newModel1.endDate_5 != null) saved.endDate_5 = newModel1.endDate_5;
                    if (newModel1.endDate_6 != null) saved.endDate_6 = newModel1.endDate_6;
                    if (newModel1.group_code != null) saved.group_code = newModel1.group_code;
                    if (newModel1.hotel_flight_name_1 != null) saved.hotel_flight_name_1 = newModel1.hotel_flight_name_1;
                    if (newModel1.hotel_flight_name_2 != null) saved.hotel_flight_name_2 = newModel1.hotel_flight_name_2;
                    if (newModel1.hotel_flight_name_3 != null) saved.hotel_flight_name_3 = newModel1.hotel_flight_name_3;
                    if (newModel1.hotel_flight_name_4 != null) saved.hotel_flight_name_4 = newModel1.hotel_flight_name_4;
                    if (newModel1.hotel_flight_name_5 != null) saved.hotel_flight_name_5 = newModel1.hotel_flight_name_5;
                    if (newModel1.hotel_flight_name_6 != null) saved.hotel_flight_name_6 = newModel1.hotel_flight_name_6;
                    if (newModel1.id_data != null) saved.id_data = Convert.ToInt32(newModel1.id_data);
                    if (newModel1.invoice_number != null) saved.invoice_number = newModel1.invoice_number;
                    if (newModel1.jenis_transaksi != null) saved.jenis_transaksi = newModel1.jenis_transaksi;
                    if (newModel1.name != null) saved.name = newModel1.name;
                    if (newModel1.no_reg != null) saved.no_reg = newModel1.no_reg;
                    if (newModel1.qty != null) saved.qty = newModel1.qty;
                    if (newModel1.startDate_1 != null) saved.startDate_1 = newModel1.startDate_1;
                    if (newModel1.startDate_2 != null) saved.startDate_2 = newModel1.startDate_2;
                    if (newModel1.startDate_3 != null) saved.startDate_3 = newModel1.startDate_3;
                    if (newModel1.startDate_4 != null) saved.startDate_4 = newModel1.startDate_4;
                    if (newModel1.startDate_5 != null) saved.startDate_5 = newModel1.startDate_5;
                    if (newModel1.startDate_6 != null) saved.startDate_6 = newModel1.startDate_6;
                    if (newModel1.tax_invoice_number != null) saved.tax_invoice_number = newModel1.tax_invoice_number;
                    if (newModel1.vendor_code != null) saved.vendor_code = Convert.ToInt32(newModel1.vendor_code);
                    if (newModel1.vendor_name != null) saved.vendor_name = newModel1.vendor_name;
                    if (newModel1.wbs_no != null) saved.wbs_no = newModel1.wbs_no;
                ...
                List<int?> duration = new List<int?>();
                List<string> destination = new List<string>();
                List<Departure_City> From = newList[loop].from;
                List<int?> amount = new List<int?>();


                start_date.Add(Convert.ToDateTime(newList[loop].invoice.startDate_1).ToString("dd MMM yyyy"));
                start_time.Add(Convert.ToDateTime(newList[loop].invoice.startDate_1).ToString("hh:mm tt"));

                end_date.Add(Convert.ToDateTime(newList[loop].invoice.endDate_1).ToString("dd MMM yyyy"));
                end_time.Add(Convert.ToDateTime(newList[loop].invoice.endDate_1).ToString("hh:mm tt"));

                destination.Add(newList[loop].invoice.destination_1);
                amount.Add(newList[loop].invoice.amount_1);

                duration.Add(newList[loop].invoice.Duration_1);
                if (newList[loop].invoice.hotel_flight_name_1 != null) hotel_ticket.Add(newList[loop].invoice.hotel_flight_name_1);
                else hotel_ticket.Add(" ");

                if (newList[loop].invoice.startDate_2 != null)
                {
                    start_date.Add(Convert.ToDateTime(newList[loop].invoice.startDate_2).ToString("dd MMM yyyy"));
                    start_time.Add(Convert.ToDateTime(newList[loop].invoice.startDate_2).ToString("hh:mm tt"));

                    end_date.Add(Convert.ToDateTime(newList[loop].invoice.endDate_2).ToString("dd MMM yyyy"));
                    end_time.Add(Convert.ToDateTime(newList[loop].invoice.endDate_2).ToString("hh:mm tt"));

                    amount.Add(newList[loop].invoice.amount_2);
                    destination.Add(newList[loop].invoice.destination_2);

                    duration.Add(newList[loop].invoice.Duration_2);

                    if (newList[loop].invoice.hotel_flight_name_2 != null) hotel_ticket.Add(newList[loop].invoice.hotel_flight_name_2);
                    else hotel_ticket.Add(" ");
                }

                if (newList[loop].invoice.startDate_3 != null)
                {
                    start_date.Add(Convert.ToDateTime(newList[loop].invoice.startDate_3).ToString("dd MMM yyyy"));
                    start_time.Add(Convert.ToDateTime(newList[loop].invoice.startDate_3).ToString("hh:mm tt"));

                    end_date.Add(Convert.ToDateTime(newList[loop].invoice.endDate_3).ToString("dd MMM yyyy"));
                    end_time.Add(Convert.ToDateTime(newList[loop].invoice.endDate_3).ToString("hh:mm tt"));

                    amount.Add(newList[loop].invoice.amount_3);
                    destination.Add(newList[loop].invoice.destination_3);

                    duration.Add(newList[loop].invoice.Duration_3);
                    if (newList[loop].invoice.hotel_flight_name_3 != null) hotel_ticket.Add(newList[loop].invoice.hotel_flight_name_3);
                    else hotel_ticket.Add(" ");
                }

                if (newList[loop].invoice.startDate_4 != null)
                {
                    start_date.Add(Convert.ToDateTime(newList[loop].invoice.startDate_4).ToString("dd MMM yyyy"));
                    start_time.Add(Convert.ToDateTime(newList[loop].invoice.startDate_4).ToString("hh:mm tt"));

                    end_date.Add(Convert.ToDateTime(newList[loop].invoice.endDate_4).ToString("dd MMM yyyy"));
                    end_time.Add(Convert.ToDateTime(newList[loop].invoice.endDate_4).ToString("hh:mm tt"));

                    amount.Add(newList[loop].invoice.amount_4);
                    destination.Add(newList[loop].invoice.destination_4);

                    duration.Add(newList[loop].invoice.Duration_4);
                    if (newList[loop].invoice.hotel_flight_name_4 != null) hotel_ticket.Add(newList[loop].invoice.hotel_flight_name_4);
                    else hotel_ticket.Add(" ");
                }

                if (newList[loop].invoice.startDate_5 != null)
                {
                    start_date.Add(Convert.ToDateTime(newList[loop].invoice.startDate_5).ToString("dd MMM yyyy"));
                    start_time.Add(Convert.ToDateTime(newList[loop].invoice.startDate_5).ToString("hh:mm tt"));

                    end_date.Add(Convert.ToDateTime(newList[loop].invoice.endDate_5).ToString("dd MMM yyyy"));
                    end_time.Add(Convert.ToDateTime(newList[loop].invoice.endDate_5).ToString("hh:mm tt"));

                    amount.Add(newList[loop].invoice.amount_5);
                    destination.Add(newList[loop].invoice.destination_5);

                    duration.Add(newList[loop].invoice.Duration_5);
                    if (newList[loop].invoice.hotel_flight_name_5 != null) hotel_ticket.Add(newList[loop].invoice.hotel_flight_name_5);
                    else hotel_ticket.Add(" ");
                }

                if (newList[loop].invoice.startDate_6 != null)
                {
                    start_date.Add(Convert.ToDateTime(newList[loop].invoice.startDate_6).ToString("dd MMM yyyy"));
                    start_time.Add(Convert.ToDateTime(newList[loop].invoice.startDate_6).ToString("hh:mm tt"));

                    end_date.Add(Convert.ToDateTime(newList[loop].invoice.endDate_6).ToString("dd MMM yyyy"));
                    end_time.Add(Convert.ToDateTime(newList[loop].invoice.endDate_6).ToString("hh:mm tt"));

                    amount.Add(newList[loop].invoice.amount_6);
                    destination.Add(newList[loop].invoice.destination_6);

                    duration.Add(newList[loop].invoice.Duration_6);
                    if (newList[loop].invoice.hotel_flight_name_6 != null) hotel_ticket.Add(newList[loop].invoice.hotel_flight_name_6);
                    else hotel_ticket.Add(" ");
                }
                ...
                gfx.DrawLine(POLine, 85, 320 + gap_now, 82, 340 + gap_now);
                gfx.DrawString("VENDOR ID", title, XBrushes.DarkBlue, 95, 323 + gap_now, XStringFormats.TopLeft);
                gfx.DrawString(newList[loop].invoice.vendor_code.ToString(), body, XBrushes.Black, 95, 343 + gap_now, XStringFormats.TopLeft);

                gfx.DrawRectangle(XBrushes.GhostWhite, 302, 320 + gap_now, 210, 20);
                gfx.DrawLine(POLine, 305, 320 + gap_now, 302, 340 + gap_now);
                gfx.DrawString("VENDOR NAME", title, XBrushes.DarkBlue, 315, 323 + gap_now, XStringFormats.TopLeft);
                gfx.DrawString(newList[loop].invoice.vendor_name.ToUpper(), body, XBrushes.Black, 315, 343 + gap_now, XStringFormats.TopLeft);

                string trans = "";
                if (newList[loop].invoice.jenis_transaksi.Contains("hotel")) trans = "Hotel";
                else if (newList[loop].invoice.jenis_transaksi.Contains("ticket")) trans = "Airlines";

                gfx.DrawRectangle(XBrushes.GhostWhite, 82, 365 + gap_now, 430, 20);
                gfx.DrawLine(POLine, 85, 365 + gap_now, 82, 385 + gap_now);
                gfx.DrawString(newList[loop].invoice.jenis_transaksi.ToUpper() + " DETAILS", title, XBrushes.DarkBlue, 95, 368 + gap_now, XStringFormats.TopLeft);

                gfx.DrawString(trans, subtitle, XBrushes.Black, 95, 388 + gap_now, XStringFormats.TopLeft);
                gfx.DrawString("Date", subtitle, XBrushes.Black, 300, 388 + gap_now, XStringFormats.TopLeft);
                if (newList[loop].invoice.jenis_transaksi.Contains("hotel")) gfx.DrawString("Day(s)", subtitle, XBrushes.Black, 365, 388 + gap_now, XStringFormats.TopLeft);
                gfx.DrawString("Price", subtitle, XBrushes.Black, 405, 388 + gap_now, XStringFormats.TopLeft);
                for (int k = 0; k < start_date.Count(); k++)
                {

                    gfx.DrawString(newList[loop].invoice.jenis_transaksi + " " + hotel_ticket[k], body, XBrushes.Black, 95, 403 + gap_now, XStringFormats.TopLeft);
                    gfx.DrawString(start_date[k], body, XBrushes.Black, 300, 403 + gap_now, XStringFormats.TopLeft);
                    if (newList[loop].invoice.jenis_transaksi.Contains("hotel")) gfx.DrawString(duration[k].ToString(), body, XBrushes.Black, 370, 403 + gap_now, XStringFormats.TopLeft);
                    gfx.DrawString("Rp. " + Convert.ToInt32(amount[k]).ToString("N"), body, XBrushes.Black, 405, 403 + gap_now, XStringFormats.TopLeft);
                    gap_now = gap_now + 15;
                }
                gfx.DrawRectangle(XBrushes.LightCyan, 82, 560 + 20, 428, 30);
                gfx.DrawLine(POLine, 85, 560 + 20, 82, 560 + 50);
                gfx.DrawString("TOTAL", title, XBrushes.DarkBlue, 85 + x_pad, 564 + 25, XStringFormats.TopLeft);
                gfx.DrawString("Rp. " + Convert.ToInt32(newList[loop].invoice.amount_total).ToString("N"), body, XBrushes.Black, 405, 564 + 25, XStringFormats.TopLeft);
                ...
                gfx.DrawString(newList[loop].loged_employee.name.Trim(' '), credit, XBrushes.Gray, 385, 723 + 25, XStringFormats.TopLeft);
                gfx.DrawString("(" + newList[loop].loged_employee.code.Trim(' ') + ")", credit, XBrushes.Gray, 385, 723 + 35, XStringFormats.TopLeft);
                ...
                if (newList.Count == 1)
                {
                    ...
                    document.Save(memoryStream, false);
                    memoryStream.Position = 0;
                    if (copy) return File(memoryStream, "application/pdf", receipt.Replace(" ", "_") + "_" + newList[loop].invoice.group_code.Trim(' ') + "_" + newList[loop].invoice.jenis_transaksi.Trim(' ').ToUpper() + "_" + DateTime.Now.ToString("yyMMdd-hh-mm-tt") + "_COPY_" + ".pdf");
                    else return File(memoryStream, "application/pdf", receipt.Replace(" ", "_") + "_" + newList[loop].invoice.group_code.Trim(' ') + "_" + newList[loop].invoice.jenis_transaksi.Trim(' ').ToUpper() + "_" + DateTime.Now.ToString("yyMMdd-hh-mm-tt") + ".pdf");
                }
            }
            MemoryStream stream = new MemoryStream();
            document.Save(stream, false);
